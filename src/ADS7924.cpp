...
bool ADS7924::comms_check() {
    ...
    Log.trace(F("ADS7924 ID check: %B\n"), id.raw);
...
bool ADS7924::read(uint16_t &data, uint8_t channel) {
    ...
    uint8_t buffer[2];
    bool successful = read(buffer, (ads7924_reg_t)channel_address, 2);
    if (successful) data = (uint16_t(buffer[0]) << 4) | (buffer[1] >> 4);
    return successful;
}
 ...
 */
bool ADS7924::write_limit(uint16_t limit, uint8_t channel, bool is_high_limit) {
    ...
    return write(&low_res_limit, ads7924_reg_t(address));
}
 ...
 * @param channel: Channel to set the threshold for. (0-3)
 */
bool ADS7924::write_high_limit(uint16_t limit, uint8_t channel) { return write_limit(limit, channel, true); }
 ...
 * @param channel: Channel to set the threshold for. (0-3)
 */
bool ADS7924::write_low_limit(uint16_t limit, uint8_t channel) { return write_limit(limit, channel, false); }
 ...
 */
bool ADS7924::read_limit(uint16_t &limit, uint8_t channel, bool is_high_limit) {
    channel = constrain(channel, 0, 3);
    uint8_t channel_address = ADS7924_REGISTER::UPPER_LIMIT_0 + (channel * 2);
    if (not is_high_limit) channel_address += 1;
    bool successful = read((uint8_t *)&limit, (ads7924_reg_t)channel_address);
    limit = limit << 4;
    return successful;
}
 ...
 * @param channel: Channel to read the threshold from. (0-3)
 */
bool ADS7924::read_high_limit(uint16_t &limit, uint8_t channel) { return read_limit(limit, channel, true); }
 ...
 * @param is_high_limit: True if the high limit is being read. False for the low limit.
 */
bool ADS7924::read_low_limit(uint16_t &limit, uint8_t channel) { return read_limit(limit, channel, false); }
